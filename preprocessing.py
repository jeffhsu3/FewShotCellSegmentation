...
def get_slices(image, slice_size, steps_x, steps_y, remove_black_images, threshold):
    ...
    for x in range(0, image_array.shape[0], steps_x):
        for y in range(0, image_array.shape[1], steps_y):
            if (
                x + slice_size <= image_array.shape[0]
                and y + slice_size <= image_array.shape[1]
            ):
...
class Preprocessing:
    def __init__(
        ...
        k_shots=None,
        crop=True,
        source_dir=os.getcwd() + "/Datasets/Raw/",
    ):
    ...
    def createFewShotTargetDirs(self):
        ...
        for selection in self.selections:
            ...
            fine_tune_dir, test_dir = (
                prefix + "FinetuneSamples/",
                prefix + "TestSamples/",
            )
            ...
            for k_shot in self.k_shots:
                for dataset in self.datasets:
                    dirs = [
                        fine_tune_dir + str(k_shot) + "-shot/" + dataset + "/Image/",
                        fine_tune_dir
                        + str(k_shot)
                        + "-shot/"
                        + dataset
    ...
    def createFewShotSourceDirs(self):
        ...
        for dataset in self.datasets:
            dirs = [
                fewshot_source_dir + dataset + "/Image/",
    ...
    def getRawImagesAndGroundtruth(self, dataset):
        ...
        if dataset == "TNBC":
            ground_truth_folders = sorted(
                [
                    folder + "/"
                    for folder in os.listdir(ground_truth_prefix)
                    if folder[0] == "G"
                ]
            )
            image_folders = sorted(
                [
                    folder + "/"
                    for folder in os.listdir(image_prefix)
                    if folder[0] == "S"
                ]
            )
            ...
            for folder in ground_truth_folders:
                ground_truth_files += sorted(
                    [
                        ground_truth_prefix + folder + f
                        for f in os.listdir(ground_truth_prefix + folder)
                        if f[0] != "."
                    ]
                )
            for folder in image_folders:
                image_files += sorted(
                    [
                        image_prefix + folder + f
                        for f in os.listdir(image_prefix + folder)
                        if f[0] != "."
                    ]
                )
        else:
            ground_truth_files = sorted(
                [
                    ground_truth_prefix + f
                    for f in os.listdir(ground_truth_prefix)
                    if f[0] != "."
                ]
            )
            image_files = sorted(
                [image_prefix + f for f in os.listdir(image_prefix) if f[0] != "."]
    ...
    def getTestImagesandGroundtruth(self, dataset, fine_tune_dir):
        ...
        if dataset == "TNBC":
            ...
            for folder in image_folders:
                image_files += sorted(
                    [
                        ...
                        for f in os.listdir(image_prefix + folder)
                        if f[0] != "."
                        and f not in os.listdir(fine_tune_dir + dataset + "/Image/")
                    ]
                )
            ...
            for folder in ground_truth_folders:
                ground_truth_files += sorted(
                    [
                        ...
                        if f[0] != "."
                        and f
                        not in os.listdir(fine_tune_dir + dataset + "/Groundtruth/")
                    ]
                )
        else:
            ground_truth_files = sorted(
                [
                    ...
                    for f in os.listdir(ground_truth_prefix)
                    if f[0] != "."
                    and f not in os.listdir(fine_tune_dir + dataset + "/Groundtruth/")
                ]
            )
            ...
            image_files = sorted(
                [
                    image_prefix + f
                    for f in os.listdir(image_prefix)
                    if f[0] != "."
                    and f not in os.listdir(fine_tune_dir + dataset + "/Image/")
                ]
            )[: len(ground_truth_files)]
    ...
    def selectKRandomShots(
        self, ground_truth_files, image_files, dataset, shot, save_dir
    ):
            ...
            for i in range(0, len(ground_truth_files)):
                for image_file in image_files:
                    if len(fileCode.findall(os.path.basename(image_file))) == 1:
                        ...
                        ) == fileCode.findall(base_name[i]):
                            temp.append(image_file)
                    elif fileCode.findall(
                        os.path.basename(image_file)
                    ) == fileCode.findall(base_name[i]):
                        temp.append(image_file)
        else:
    ...
    def preprocess_Groundtruth(
        ...
        for file in ground_truth_files:
            ...
            if dataset == "ssTEM" and file.find("labels") != -1:
                ...
                ground_truth[ground_truth < 0.5] = 0
            if crop:
                preprocessed_ground_truth += get_slices(
                    ground_truth,
                    size,
                    steps_x=steps["x"],
                    steps_y=steps["y"],
    ...
    def preprocess_Images(
        ...
        if crop:
            for file in image_files:
                ...
                preprocessed_images += get_slices(
                    image,
                    size,
                    steps_x=steps["x"],
                    steps_y=steps["y"],
    ...
    def save_preprocessed_data(
        self, preprocessed_ground_truth, preprocessed_images, save_dir, target=None
    ):
        ...
        for i, sample in enumerate(preprocessed_ground_truth):
            if sample is not None:
                ...
                img = img.convert("L")
                img.save(
                    save_dir["groundtruth"] + "ground_truth{}.png".format(img_count)
                )
    ...
    def preprocess_Target_Data(
        ...
        for selection in self.selections:
            ...
            for shot in self.k_shots:
                ...
                for dataset in self.datasets:
                    save_ft_dir = {
                        "image": fine_tune_dir + "preprocessed/" + dataset + "/Image/",
                        "groundtruth": fine_tune_dir
                        + "preprocessed/"
                        + dataset
                        + "/Groundtruth/",
                    }
                    ...
                    save_test_dir = {
                        "image": test_dir + dataset + "/Image/",
                        "groundtruth": test_dir + dataset + "/Groundtruth/",
                    }
                    image_files, ground_truth_files = self.getRawImagesAndGroundtruth(
                        dataset
                    )
                    image_files, ground_truth_files = self.selectKRandomShots(
                        ground_truth_files,
                        image_files,
                        dataset,
                        shot,
                    ...
                    preprocessed_images, preprocessed_ground_truth = self.preprocess(
                        ground_truth_files,
                        image_files,
                        dataset,
                        crop=crop,
                    ...
                    self.save_preprocessed_data(
                        preprocessed_ground_truth=preprocessed_ground_truth,
                        preprocessed_images=preprocessed_images,
                        save_dir=save_ft_dir,
                    )

                    test_images, test_groundtruth = self.getTestImagesandGroundtruth(
                        dataset, fine_tune_dir
                    )
                    crop = False
                    save_dir = {
                        "image": test_dir + dataset + "/Image/",
                    ...
                    test_images, test_groundtruth = self.preprocess(
                        test_groundtruth, test_images, dataset, crop=crop
                    )
                    self.save_preprocessed_data(
                        test_groundtruth, test_images, save_dir=save_test_dir
                    )
                    crop = True
    ...
    def preprocess_Source_Data(
        ...
        for dataset in self.datasets:
            ...
            save_dir = {
                "image": prefix + "Image/",
                "groundtruth": prefix + "Groundtruth/",
            }
    ...
    def reprocessFTandTestSamples(
        self, crop_window_size=256, crop_steps_dataset=None, remove_black_images=False
    ):
        ...
        for selection in self.selections:
            ...
            for shot in self.k_shots:
                ...
                for dataset in self.datasets:
                    preprocess_dirs = [
                        fine_tune_dir + "preprocessed/" + dataset + "/Image/",
                    ...
                    fewShot_image_files = sorted(
                        [
                            fine_tune_dir + dataset + "/Image/" + f
                            for f in os.listdir(fine_tune_dir + dataset + "/Image/")
                        ]
                    )
                    fewShot_ground_truth_files = sorted(
                        [
                            fine_tune_dir + dataset + "/Groundtruth/" + f
                            for f in os.listdir(
                                fine_tune_dir + dataset + "/Groundtruth/"
                            )
                        ]
                    )
                    ...
                    test_image_files = sorted(
                        [
                            test_dir + dataset + "/Image/" + f
                            for f in os.listdir(test_dir + dataset + "/Image/")
                        ]
                    )
                    test_ground_truth_files = sorted(
                        [
                            test_dir + dataset + "/Groundtruth/" + f
                            for f in os.listdir(test_dir + dataset + "/Groundtruth/")
                        ]
                    )

                    (
                        preprocessed_fewshot_images,
                        preprocessed_fewShot_ground_truth,
                    ) = self.preprocess(
                        fewShot_ground_truth_files,
                        fewShot_image_files,
                        dataset,
                        steps=crop_steps_dataset[dataset],
                        remove_black_images=remove_black_images,
                    ...
                    (
                        ...
                        preprocessed_test_ground_truth,
                    ) = self.preprocess(
                    ...
                    assert len(preprocessed_fewshot_images) == len(
                        preprocessed_fewShot_ground_truth
                    )
                    assert len(preprocessed_test_images) == len(
                        preprocessed_test_ground_truth
                    )
                    self.save_preprocessed_data(
                        preprocessed_ground_truth=preprocessed_fewShot_ground_truth,
                        preprocessed_images=preprocessed_fewshot_images,
                        save_dir=save_ft_dir,
                    )

                    for test_image_file, test_ground_truth_file in zip(
                        test_image_files, test_ground_truth_files
                    ):
                        os.remove(test_image_file)
                        os.remove(test_ground_truth_file)

                    self.save_preprocessed_data(
                        preprocessed_ground_truth=preprocessed_test_ground_truth,
    ...
    def preprocess_Data(
        ...
        self.preprocess_Target_Data(
            crop_window_size=crop_window_size, crop_steps_dataset=crop_steps_dataset
        )
        self.preprocess_Source_Data(
            crop_window_size=crop_window_size, crop_steps_dataset=crop_steps_dataset
        )
...
if __name__ == "__main__":
    ...
    preprocessing = Preprocessing(
        selections=selections, datasets=datasets, k_shots=k_shots, target_dir=target_dir
    )
    preprocessing.reprocessFTandTestSamples(
        crop_steps_dataset=ft_crop_steps_dataset, remove_black_images=True
    )
    preprocessing.preprocess_Source_Data(
        crop_steps_dataset=source_crop_steps_dataset, remove_black_images=True
    )
