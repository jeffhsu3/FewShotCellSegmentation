...
class Cell_Segmentation_Task:
    def __init__(
        ...
        self.train_loader = DataLoader(
            self.traindataset, batch_size=self.batch_size, sampler=self.meta_sampler
        )
...
class CustomDataset(Dataset):
    def __init__(
        ...
        for dset in dataset_selection:
            ground_truth_prefix = dset + "/Groundtruth/"
            image_prefix = dset + "/Image/"
            ...
            if self.split == True:
                ...
                if self.split_type == "train_valid":
                    file_names_g = sorted(
                        [
                            self.root_dir + ground_truth_prefix + f
                            for f in os.listdir(self.root_dir + ground_truth_prefix)
                            if f[0] != "."
                        ]
                    )
                    file_names_i = sorted(
                        [
                            self.root_dir + image_prefix + f
                            for f in os.listdir(self.root_dir + image_prefix)
                            if f[0] != "."
                        ]
                    )[: len(file_names_g)]
                    ...
                    f = open(
                        f"{os.getcwd()}/Datasets/FewShot/Train_Valid_1000/valid_ids_{dset}.pickle",
                        "wb",
                    )
                    ...
                    self.ground_truth_train = sorted(
                        [
                            self.root_dir + "Train/" + ground_truth_prefix + f
                            for f in os.listdir(
                                self.root_dir + "Train/" + ground_truth_prefix
                            )
                            if f[0] != "."
                        ]
                    )
                    self.images_train = sorted(
                        [
                            self.root_dir + "Train/" + image_prefix + f
                            for f in os.listdir(self.root_dir + "Train/" + image_prefix)
                            if f[0] != "."
                        ]
                    )[: len(self.ground_truth_train)]

                    self.ground_truth_valid = sorted(
                        [
                            self.root_dir + "Valid/" + ground_truth_prefix + f
                            for f in os.listdir(
                                self.root_dir + "Valid/" + ground_truth_prefix
                            )
                            if f[0] != "."
                        ]
                    )
                    self.images_valid = sorted(
                        [
                            self.root_dir + "Valid/" + image_prefix + f
                            for f in os.listdir(self.root_dir + "Valid/" + image_prefix)
                            if f[0] != "."
                        ]
                    )[: len(self.ground_truth_valid)]

                    self.ground_truth_test = sorted(
                        [
                            self.root_dir + "Test/" + ground_truth_prefix + f
                            for f in os.listdir(
                                self.root_dir + "Test/" + ground_truth_prefix
                            )
                            if f[0] != "."
                        ]
                    )
                    self.images_test = sorted(
                        [
                            self.root_dir + "Test/" + image_prefix + f
                            for f in os.listdir(self.root_dir + "Test/" + image_prefix)
                            if f[0] != "."
                        ]
                    )[: len(self.ground_truth_test)]
    ...
    def selectKshots(self, splitFactor=1):
        ...
        samples = np.random.choice(
            indicies, self.k_shot if self.k_shot != None else dataset_size
        )
